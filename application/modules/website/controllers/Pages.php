...
class Pages extends MY_Controller 
{
    ...
    public function update_profile_image($user_id)
    {
      if((isset($_FILES['profile_image']['name'])) && ($_FILES['profile_image']['name']!='')){
        $actual_image_name=$_FILES['profile_image']['name'];
        ...
        if ( ! $this->upload->do_upload('profile_image'))
        {
          ...
          $this->session->set_flashdata('error', 'Your image not been uploaded,please upload again!');
          //redirect('website/Cms/my_profile');
          redirect($this->agent->referrer());
        }  
      }
        ...
        {
          $data['image']=$actual_image;
          unlink('./uploads/user_profile_images/'.$old_profile_image);
        }
        else
        {
          $data['image']=$old_profile_image;
        }
        $condition['id'] = $user_id;
        $update_data = $this->BlankModel->updateData("users", $data, $condition);
